...
기존 RAG(컨텍스트 수집·주입) 대신 **OpenAI Function Calling**을 사용하여, GPT가 필요 시 도메인 함수(예: `search_recipes`)를 호출하고 그 결과를 반영한 최종 응답을 생성하도록 한다.

### 5.1 실행 시나리오 (End-to-End)

1. **유저 요청 → Producer**: 클라이언트가 `POST /api/v1/chatbot/messages` 등으로 요청. Producer는 `streamChannelId` 생성 후 Kafka(`CHATBOT_REQUESTS`)에 이벤트 발행. 페이로드에 **userIngredientIds**, **favoriteIngredientIds**를 포함하여 Consumer에서 레시피 매칭에 활용한다.
2. **Consumer 수신 → ProcessChatHandler**: Kafka 메시지 수신 후 `ProcessChatHandler`가 실행된다. userId, sessionId, conversationId, 자연어 입력, userIngredientIds, favoriteIngredientIds 등을 payload로 전달받는다.
3. **GPT Function Call**: OpenAI Chat Completions API에 **tools**(함수 정의)를 넘기고 스트리밍 호출. GPT가 레시피 검색이 필요하다고 판단하면 `search_recipes`를 호출한다. 예: `search_recipes({ keywords: ['간단','저녁'], maxCookTime: 30 })`.
4. **SearchRecipesHandler**: Function call 인자를 받아 **PostgreSQL(Prisma)**로 레시피 검색을 수행하고, userIngredientIds·favoriteIngredientIds 기반 **매칭 점수**를 계산한다. 결과는 **`RecipeMatchResult[]`** 형태로 GPT에 반환한다. 각 항목에는 기본 레시피 정보와 **matchContext**(매칭 점수, 보유·부족 재료 상세, 즐겨찾기 재료 사용 여부, 추천 이유 등)가 포함되어, GPT가 "왜 이 레시피를 추천하는지"를 사용자에게 설명할 수 있도록 한다.
5. **GPT 최종 응답**: 함수 호출 결과(RecipeMatchResult[])를 컨텍스트로 받은 GPT가, matchContext(보유 재료 매칭·부족 재료·즐겨찾기 재료 사용 등)를 활용해 **추천 이유를 설명할 수 있는** 자연어 응답(예: "보유 재료로 대부분 만들 수 있는 김치찌개를 추천드립니다. 부족한 재료는 두부뿐이에요")과 함께 **suggestedRecipes: RecipeMatchResult[]**를 구조화된 데이터로 내려준다.
6. **Redis 발행 → Producer SSE**: Consumer는 `ChatbotStreamEvent`(type: `chunk` | `tool_call` | `done` | `error`, 본문·suggestedRecipes 포함)를 Redis 채널 `chatbot:stream:{streamChannelId}`에 발행. Producer는 해당 채널을 구독하여 SSE(`data: {JSON}\n\n`)로 클라이언트에 전달한다.
7. **MongoDB 로깅**: 스트림 종료 후 **ChatbotLog**에 대화 턴, 요청/응답 요약, suggestedRecipes 등 필요한 필드를 Mongoose로 저장한다.

### 5.2 Function 정의·핸들러 설계 원칙

- **함수 정의**: OpenAI `tools` 배열에 `search_recipes` 등 도메인 함수의 이름·설명·parameters(JSON Schema)를 명시한다. 예: `keywords: string[]`, `maxCookTime?: number` 등.
...
- **검색·매칭**: `SearchRecipesHandler`는 Prisma로 Recipe·RecipeIngredient·Ingredient를 조회하고, userIngredientIds·favoriteIngredientIds와의 일치도로 매칭 점수를 계산한 뒤, 상위 N건을 **`RecipeMatchResult[]`**로 반환한다. 각 RecipeMatchResult는 기본 레시피 정보(id, title, description, difficulty, cookTime 등)와 **RecipeMatchContext**(matchScore, ingredientMatch: matchedIngredients/missingIngredients/missingOptionalIngredients·stats, usesFavoriteIngredients, favoriteIngredientsUsed, recommendationReasons 등)를 포함하여, GPT가 추천 이유를 설명할 수 있도록 한다.
...
### 5.2.1 Tool 반환 타입: RecipeMatchResult (SearchRecipesHandler)

GPT가 추천 이유를 설명할 수 있도록, `SearchRecipesHandler`의 반환값은 **RecipeMatchResult[]**로 한다. 각 항목은 기본 레시피 정보와 매칭 컨텍스트를 포함한다.

- **RecipeMatchResult**: 기본 레시피 정보(id, title, description, difficulty, cookTime, imageUrl, servings, viewCount, isPublished, createdAt) + **matchContext: RecipeMatchContext**
- **RecipeMatchContext**:
  - **matchScore**: 0–100 매칭 점수
  - **ingredientMatch**: matchedIngredients(보유 재료 중 이 레시피에 사용되는 것), missingIngredients(보유하지 않은 필수 재료), missingOptionalIngredients(보유하지 않은 선택 재료), stats(totalRequired, matchedRequired, totalOptional, matchedOptional)
  - **usesFavoriteIngredients**, **favoriteIngredientsUsed**: 즐겨찾기 재료 사용 여부 및 사용된 즐겨찾기 재료 이름 목록
  - **recommendationReasons** (선택): 추천 이유 문구 배열
- **IngredientDetail**: id, name, amount?, unit?, category (재료 상세 표시용)

구현 시 위 타입을 shared 또는 consumer 도메인에 정의하고, Handler는 이 구조로 GPT에 전달하여 "왜 이 레시피인지" 설명 가능하게 한다.

### 5.3 SSE·로그와의 연동

- **ChatbotStreamEvent와 Function Calling 이벤트**: `ChatbotStreamEvent`에는 Function Calling 관련 이벤트를 추가한다. 예: `type: 'tool_call'` — payload에 `functionName`(예: `search_recipes`), 필요 시 `arguments` 요약 또는 `status`(예: `start` | `complete`)를 담아, 클라이언트가 “레시피 검색 중…”, “함수 실행 완료” 등 상태를 구분할 수 있게 한다. 기존 `chunk`(스트리밍 텍스트), `done`(최종 payload에 suggestedRecipes 등 포함), `error`와 함께 스트림 구간별로 발행한다.
- **클라이언트 피드백**: 실제 Function Calling이 일어나는 시점에 Consumer가 `tool_call` 이벤트를 Redis에 발행하면, Producer가 SSE로 그대로 전달하므로 **클라이언트는 서버가 함수(레시피 검색 등)를 실행 중임을 실시간으로 피드백 받을 수 있다.** UI에서 로딩/스피너 또는 “레시피를 찾고 있어요” 같은 메시지를 표시하는 데 활용한다.
...
- **ChatbotLog**: 대화 턴별로 요청 메시지, GPT 최종 응답 텍스트, 호출된 function 목록·인자·결과 요약, suggestedRecipes(RecipeMatchResult[] 또는 해당 도메인 결과)를 Mongoose 스키마에 맞게 저장한다. TTL·인덱스 정책은 `schema.md` 및 Consumer 스키마 정의를 따른다.

...
4. **SearchRecipesHandler**: Function call 인자를 받아 **PostgreSQL(Prisma)**로 레시피 검색을 수행하고, userIngredientIds·favoriteIngredientIds 기반 **매칭 점수**를 계산한다. 결과는 `RecipeSummary[]` 형태로 GPT에 반환된다.
5. **GPT 최종 응답**: 함수 호출 결과를 컨텍스트로 받은 GPT가 자연어 응답(예: "김치찌개를 추천드립니다")과 함께 **suggestedRecipes: RecipeSummary[]**를 구조화된 데이터로 내려준다.
...
- **검색·매칭**: `SearchRecipesHandler`는 Prisma로 Recipe·RecipeIngredient·Ingredient를 조회하고, userIngredientIds·favoriteIngredientIds와의 일치도로 매칭 점수를 계산한 뒤, 상위 N건을 `RecipeSummary[]`로 반환한다.
...
- **ChatbotLog**: 대화 턴별로 요청 메시지, GPT 최종 응답 텍스트, 호출된 function 목록·인자·결과 요약, suggestedRecipes(또는 해당 도메인 결과)를 Mongoose 스키마에 맞게 저장한다. TTL·인덱스 정책은 `schema.md` 및 Consumer 스키마 정의를 따른다.

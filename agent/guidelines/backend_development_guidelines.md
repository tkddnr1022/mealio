...
기존 RAG(컨텍스트 수집·주입) 대신 **OpenAI Function Calling**을 사용하여, GPT가 필요 시 도메인 함수(예: `search_recipes`, `get_user_ingredients`)를 호출하고 그 결과를 반영한 최종 응답을 생성하도록 한다.
...
- **검색·매칭**: `SearchRecipesHandler`는 Prisma로 Recipe·RecipeIngredient·Ingredient를 조회하고, (필요한 경우) 유저 재료/선호 재료와의 일치도로 매칭 점수를 계산한 뒤 상위 N건을 `RecipeSummary[]`로 반환한다. 이때 유저 재료 컨텍스트는 `get_user_ingredients` tool 결과를 통해 간접적으로 주입되며, **Producer가 Kafka 이벤트에 userIngredientIds 배열을 실어 나르지 않는다.**

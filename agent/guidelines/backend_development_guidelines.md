...
1. **유저 요청 → Producer**: 클라이언트가 `POST /api/v1/chatbot/messages` 등으로 요청. Producer는 `streamChannelId` 생성 후 Kafka(`CHATBOT_REQUESTS`)에 이벤트 발행한다. 이때 페이로드에는 **userId, sessionId, conversationId, 자연어 입력 등 최소 메타데이터만 포함**하고, **대용량 도메인 배열은 포함하지 않는다**(통신 비용 절감 및 관심사 분리).
2. **Consumer 수신 → ProcessChatHandler**: Kafka 메시지 수신 후 `ProcessChatHandler`가 실행된다. payload에는 userId, sessionId, conversationId, 자연어 입력 등만 포함되며, **유저 재료·레시피 등 도메인 데이터는 필요할 때마다 tool call을 통해 조회**한다.
...
4. **도메인 Tool Handlers**:
   - **UserIngredientsHandler (`get_user_ingredients`)**: MongoDB의 `UserIngredient` 컬렉션에서 유저가 보유/관심 있는 재료 ID를 조회한 뒤, **PostgreSQL(Prisma) `Ingredient` 마스터와 조인**하여 `[{ id, name, ... }]` 형태로 반환한다. 이때, **재료 마스터는 Redis 등으로 적절히 캐싱**하여 id → name 변환을 반복적으로 수행하지 않도록 한다.
...
- **검색·매칭**: `SearchRecipesHandler`는 Prisma로 Recipe·RecipeIngredient·Ingredient를 조회하고, (필요한 경우) 유저 재료/선호 재료와의 일치도로 매칭 점수를 계산한 뒤 상위 N건을 `SuggestedRecipe[]`로 반환한다. 이때 유저 재료 컨텍스트는 `get_user_ingredients` tool 결과를 통해 간접적으로 주입되며, **Producer가 Kafka 이벤트에 userIngredientIds 배열을 실어 나르지 않는다.**
...
| 구성 요소 | 파일 | 역할 |
|----------|------|------|
| **대화 메시지 구성** | `chatbot-requests/context/conversation.manager.ts` | `buildMessagesForGpt(previousTurns, newUserMessage)`로 GPT용 메시지 배열 생성. 시스템 프롬프트 + 이전 턴(user/assistant) + 새 사용자 메시지 순서로 구성. |
| **호출처** | `chatbot-requests/handlers/ProcessChatHandler.ts` | GPT 스트리밍 호출 전 `buildMessagesForGpt`를 호출해 `messages`를 얻고, OpenAI API에 전달. |
...
- **목적**: 동일 `conversationId`(또는 `sessionId`) 내 이전 user/assistant 메시지를 GPT 컨텍스트에 포함해 연속 대화를 구현.
- **저장소**: MongoDB **ChatbotLog** (`chatbot_logs`). `SaveChatLogHandler`가 턴 종료 시 user 턴·assistant 턴을 각각 `role`, `message`, `context.conversationId`/`sessionId`와 함께 저장.
- **조회 설계(확장 방향)**: ProcessChatHandler에서 `conversationId`(또는 `sessionId`)가 있으면, ChatbotLog를 `userId`·`conversationId`(또는 `sessionId`) 기준으로 조회하고 `createdAt` 순 정렬 후 **최근 N턴**만 취해 `PreviousTurn[]`로 변환. N은 토큰/윈도우 제한을 고려해 정한다(예: 최근 10~20턴). 조회 전용 메서드는 `chatbot-log.repository` 또는 ChatbotLog Model 직접 사용으로 구현.
...
- Payload에 `conversationId`, `sessionId`가 포함되어 있으면, 해당 ID로 ChatbotLog에서 이전 턴을 조회한다.

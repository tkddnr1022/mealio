...
- **Handler 레이어**: Handler는 Kafka 토픽을 직접 발행하지 않는다. DB 반영 후 Producer 캐시를 무효화할 필요가 있으면 **CacheInvalidationRequestService**에 요청만 한다 (예: `requestUserProfileInvalidation(userId)`, `requestUserIngredientInvalidation(userId)`). 실제 토픽 발행은 해당 서비스에서 수행하며, cache-invalidation 토픽을 구독하는 Processor·RedisInvalidationHandler가 Redis DEL을 실행한다.

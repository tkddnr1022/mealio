...
- **목적**: 디자인 파일의 토큰(변수)·텍스트·색·이펙트 스타일을 체계적으로 문서화하고, 코드 토큰(`design_tokens.json` 등)과의 갭·리스크를 드러낸다.
- **범위**: Figma 변수, Figma 스타일(텍스트/색/이펙트), 화면별 사용처 요약, 통합 병합, 구조·대비·중복 검토.
...
통합 표와 `design_tokens.json`을 나란히 두고 아래를 점검한다. 상세 논점·해결 방향 예시는 `figma-variables-and-styles.md`의 「토큰·스타일 검토」 절을 참고한다.
...
- 갱신 시에는 기존 **권장 수정사항 섹션을 항상 먼저 재검토**하고, 각 항목의 해결 여부를 확인한다.
- 기존 항목이 실제로 해결되었다면 해당 항목은 **목록에서 제거**하고, 아직 미해결인 항목만 유지한다.
...
5. `design_tokens.json`과 대조해 **이름·의미·SSOT·대비·복잡도** 체크리스트 적용.  
6. 갱신 시 기존 `권장 수정사항` 목록을 먼저 확인해 해결 여부를 점검하고, **해결된 항목은 제거**·미해결 항목만 유지한다.
7. 필요 시 `figma-variables-and-styles.md`의 검토 절을 확장하거나 이 가이드에 교훈을 반영해 업데이트한다.
8. 갱신 작업에서 **실질 변경점(신규/수정/삭제 토큰·스타일, 사용처 변경, 검토 결과 변경)**이 없으면 해당 회차 업데이트는 생략할 수 있다.

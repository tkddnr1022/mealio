...
| client/src/lib/config/env.ts | `NEXT_PUBLIC_*` 환경 변수 로드·검증(수동 파서). `apiBaseUrl`, `apiPrefix`, `authCookieName`, `refreshCookieName`, `observabilityEndpoint`, `runtime`/`isProduction`/`isDevelopment`, `validationErrors`·`assertEnv` |

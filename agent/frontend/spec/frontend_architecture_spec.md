...
| **ISR** | `export const revalidate`가 있는 레시피 메인·필터·재료 필터 (`/recipe`, `/recipe/filter`, `/ingredient/filter`). 레시피 상세(`/recipe/[id]`)는 `revalidate = false` 온디맨드 ISR |
...
| (main) · 레시피 탭 | `/recipe/[id]` | `(main)/recipe/[id]/page.tsx` | 온디맨드 ISR | `revalidate = false`, `generateStaticParams`(size 10). 데이터 변경 시 `POST /api/revalidate` → `revalidatePath('/recipe/{id}')` |
...
| `/recipe/[id]` | RecipeDetailPage | 온디맨드 ISR | `revalidate = false`, `generateStaticParams`(size 10). `POST /api/revalidate` 웹훅으로 `revalidatePath` |
...
| client/src/app/api/revalidate/route.ts | 온디맨드 ISR 웹훅(`POST`). 본문 `{ secret, id }` 검증 후 `revalidatePath('/recipe/{id}')` 호출. `secret`은 서버 환경 변수 `REVALIDATE_SECRET`과 일치해야 함. `401`(secret 불일치), `400`(id 유효성 실패), `500`(secret 미설정) |
...
| client/src/lib/config/sentry.config.ts | Sentry init 옵션(`getSentryInitOptions`, `isSentryEnabled`) — `env` 파생 |
...
| client/src/lib/policy/cache.policy.ts | React Query `QUERY_DEFAULTS`·`QUERY_CACHE`(§4 성능 예산) |

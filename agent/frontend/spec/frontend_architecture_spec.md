...
| **ISR** | fetch `next.revalidate`·`next.tags`(Data Cache)로 캐시하는 레시피 메인·필터·재료 필터 (`/recipe`, `/recipe/filter`, `/ingredient/filter`). 레시피 상세(`/recipe/[id]`)는 `isrRecipeDetailFetch` 온디맨드 ISR |
...
| client/src/lib/constants/cache-tags.constants.ts | Data Cache 태그 계약 상수(`CACHE_TAGS`, `CACHE_TAG_PATTERN`, `CACHE_TAG_LIMITS`)·`recipeDetailTag`·`recipeMutationRevalidateTags`·`/api/revalidate` 본문 `tags` 검증(`isValidCacheTag`, `parseRevalidateTags`) |
...
| client/src/lib/policy/cache.policy.ts | React Query `QUERY_DEFAULTS`·`QUERY_CACHE`(§4 성능 예산). Next.js ISR Data Cache용 `ISR_*_FETCH`·`isrRecipeDetailFetch`·`ISR_FETCH_REVALIDATE_SEC` |

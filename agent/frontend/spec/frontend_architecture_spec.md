...
| client/src/lib/config/env.ts | `NEXT_PUBLIC_*` 환경 변수 로드·검증(수동 파서). `apiBaseUrl`, `apiPrefix`, `sentryDsn`, `gaMeasurementId`, `runtime`/`isProduction`/`isDevelopment`, `validationErrors`·`assertEnv` |
| client/src/lib/config/auth.config.ts | Access·Refresh HttpOnly 쿠키 이름(`ACCESS_TOKEN_COOKIE_NAME`, `REFRESH_TOKEN_COOKIE_NAME`) — OpenAPI·OAuth Set-Cookie 계약 고정 상수 |

...
- **내부 경로인지** 한 곳에서만 판별한다. 저장소에 둔 헬퍼 이름·역할·경로는 명세 **`../spec/frontend_architecture_spec.md` §5.7(유틸)** 을 본다. 지침에서 잡는 규칙은 개념만: **같은 오리진 앱 라우트로 쓸 단일 슬래시 경로**를 `Link` 대상으로 분류하고, 프로토콜 상대 URL(`//…`)·외부 절대 URL·해시 전용은 별도 외부/앵커 경로로 분류한다.
...
- **OAuth·백엔드 진입 URL**: **전체 문서 네비게이션으로 백엔드 `GET`을 타는** 흐름은 문서 네비게이션 경로로 직접 진입한다. 계약·페이지 경로는 명세와 백엔드 OAuth 가이드를 보고, 구현 패턴은 **§10**을 따른다.
- **Storybook**: 라우터 밖에서 실제 `href` 이동이 캔버스를 깨뜨릴 수 있으면, 컴포넌트가 제공하는 **`preventLinkNavigation`**(또는 동등 옵션)으로 기본 이동을 막고 상태만 바꾼다.

### 1.3 `page.tsx`의 Server Component 원칙 (기본값: Server Component)

`page.tsx`는 기본적으로 Server Component로 유지한다.  
`page.tsx`를 Client Component로 전환할 때는 명시적 승인 절차를 거친다.
...
- **필수 본문 데이터**(`default` export): 위 헬퍼를 사용한다. 재검증 실패 시 stale 유지 목표에 맞춰 실패를 상위로 전파하는 흐름을 유지한다.
...
1. **캐시가 단일 진실 공급원이다.** 화면은 쿼리 캐시를 직접 구독하여 렌더링한다.
2. **예측 가능한 command는 캐시를 직접 업데이트한다.** 인터랙션 시 `setQueryData`로 쿼리 캐시를 optimistic update하고, 에러 시 롤백한다.
3. **성공 시에는 stale 마킹을 적용한다.** `invalidateQueries({ refetchType: 'none' })`으로 캐시를 stale 상태로 표시하고, 다음 자연스러운 refetch 시점(페이지 이동·마운트 등)에 Consumer 처리가 완료된 실제 데이터를 반영해 최종 일관성을 보장한다.
4. 이 전략은 **쿼리의 결과를 예측할 수 있고, 실패 가능성이 낮은 command API**에 한정한다.
5. `onMutate`에서 **`cancelQueries`를 반드시 호출**하여 진행 중인 refetch와의 경합을 방지한다.

#### 로컬 상태와의 관계

- 화면은 `useQuery().data`를 직접 렌더링한다.
...
- **타입 규칙(필수)**: 스토리는 `Meta<typeof Component>` / `StoryObj<typeof meta>` + `satisfies` 패턴으로 작성해 **프로퍼티 타입 추론**을 강제한다. `args`가 실제 컴포넌트 prop 타입에서 자동 검증되는 흐름을 유지한다.
...
### 9.2 빈 `name` 폴백 (유틸 내부 처리 원칙)

- **`name`이 공백뿐이면** 유틸 구현 내부에서 타입별 기본 이름으로 정규화한 뒤 위 규칙을 적용한다.
- 기본 이름 매핑: `button` → 동작, `link` → 페이지, `input` → 내용, `section` → 콘텐츠, `image` → 표시, `generic` → 항목. 구현 상수(예: 폴백 테이블)와 반드시 동기화한다.

### 9.3 컴포넌트 공개 API 규칙

- **`aria-label`은 기존 의미 prop 기반으로 조합한다.** `placeholder`, 링크/버튼에 보이는 문구, 칩의 `label`, 재료 `name` 등을 조합해 `buildAriaLabel`에 전달한다.
- 네이티브/래퍼가 `aria-label`을 허용하는 경우, `Omit<..., 'aria-label'>` 등으로 내부 조합 경로를 고정하고 `buildAriaLabel` 적용 지점을 컴포넌트 내부로 일원화한다(예: 검색 바).

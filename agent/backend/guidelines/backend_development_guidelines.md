...
4. **도메인 Tool Handlers**:
   ...
   - **SearchRecipesHandler (`search_recipes`)**: Function call 인자를 받아 **PostgreSQL(Prisma)** 로 레시피 검색을 수행한다. `ingredientIds?: string[]` 는 **optional** 로 두어, 유저 재료와 무관한 일반 검색(키워드·시간·난이도 기준)도 지원한다. 유저 재료 기반 추천이 필요한 경우에는, GPT가 먼저 `get_user_inventory` 를 호출해 재료 컨텍스트를 얻고, 그 결과(또는 그 안의 id 배열)를 활용해 `search_recipes` 를 호출하도록 프롬프트로 유도한다. 검색 결과는 `SuggestedRecipe[]` 형태로 GPT에 반환한다.
...
- **함수 정의 (OpenAI tools)**: OpenAI `tools` 배열에 `search_recipes`, `get_user_inventory` 등 도메인 함수의 이름·설명·parameters(JSON Schema)를 명시한다.

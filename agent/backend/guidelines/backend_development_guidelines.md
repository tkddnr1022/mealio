...
- **검색·매칭**: `SearchRecipesHandler`는 pgvector ANN으로 의미 기반 후보를 넓게 수집한 뒤, hard constraint(기피 재료·published)를 적용하고 composite score로 재랭킹해 상위 N건을 `SearchedRecipe[]`로 반환한다. 유저 재료 컨텍스트는 `get_user_inventory` tool 결과의 `ingredientIds`로 `inventoryMatchScore`에 반영하며, Producer 이벤트는 최소 메타데이터 계약을 따른다.
...
- **tool call 선택성**: Prompt 설계 시 “필요하다고 판단될 때에만 `tools` 를 사용하고, 대화 컨텍스트와 프롬프트 기반 답변 경로도 함께 사용한다”는 지침을 명시해 **자연스러운 대화 흐름**을 보장한다.

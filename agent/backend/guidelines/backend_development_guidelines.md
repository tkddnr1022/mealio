...
3. **GPT Function Call (선택적)**: OpenAI Chat Completions API에 **tools**(도메인 함수 정의)를 넘기고 스트리밍 호출한다. `tool_choice: 'auto'` 를 사용하여 **모든 tool call 여부·횟수는 GPT가 판단**하게 한다. 예를 들어,
   ...
   - 레시피 검색이 필요하다고 판단하면 `search_recipes` 를 호출한다. 이때 사용자 발화에서 해석한 조건(키워드, 최대 조리시간, 포함/제외 재료명, 카테고리 id 등)은 **반드시 `search_recipes` 인자에 명시적으로 전달**한다. 예: `search_recipes({ keywords: ['간단','저녁'], maxCookTime: 30, avoidIngredients: ['우유'] })`.
4. **도메인 Tool Handlers**:
   ...
   - **SearchRecipesHandler (`search_recipes`)**: Function call 인자를 받아 **PostgreSQL(Prisma)** 로 레시피 검색을 수행한다. `ingredientIds` 는 optional로 유지하되, `maxCookTime`, `mustHaveIngredients`, `avoidIngredients`, `recipeCategoryIds`, `ingredientCategoryIds` 등 확장 필드를 함께 처리한다. 유저 재료 기반 추천이 필요한 경우에는 GPT가 먼저 `get_user_inventory` 를 호출해 재료 컨텍스트를 얻고, 그 결과 id 배열을 `search_recipes` 인자에 전달하도록 프롬프트로 유도한다. 검색 결과는 `SuggestedRecipe[]` 형태로 GPT에 반환한다.
...
- **함수 정의 (OpenAI tools)**: OpenAI `tools` 배열에 `search_recipes`, `get_user_inventory`, `get_food_categories`, `finalize_recipe_selection` 등 도메인 함수의 이름·설명·parameters(JSON Schema)를 명시한다.
  ...
  - `search_recipes` 예시: `parameters: { type: 'object', properties: { keywords: { type: 'array', items: { type: 'string' } }, ingredientIds: { type: 'array', items: { type: 'number' } }, mustHaveIngredients: { type: 'array', items: { type: 'string' } }, avoidIngredientIds: { type: 'array', items: { type: 'number' } }, avoidIngredients: { type: 'array', items: { type: 'string' } }, maxCookTime: { type: 'number' }, recipeCategoryIds: { type: 'array', items: { type: 'number' } }, ingredientCategoryIds: { type: 'array', items: { type: 'number' } } } }` 형태로 확장 필드를 함께 정의해, 파싱된 조건이 숨은 서버 상태 없이 tool 인자만으로 재현되게 한다.
...
- **검색·매칭**: `SearchRecipesHandler`는 Prisma로 Recipe·RecipeIngredient·Ingredient를 조회하고, (필요한 경우) 유저 재료/선호 재료와의 일치도로 매칭 점수를 계산한 뒤 상위 N건을 `SuggestedRecipe[]`로 반환한다. 이때 유저 재료 컨텍스트는 `get_user_inventory` tool 결과(`ingredients.owned`, `ingredients.favorite`)를 통해 간접적으로 주입되며, **Producer가 Kafka 이벤트에 owned/favorites 배열(예: `ownedIngredientIds`, `favoriteIngredientIds`)을 직접 실어 나르지 않는다.**

...
챗봇은 **OpenAI Function Calling**을 사용한다. GPT가 필요 시 도메인 함수(예: `search_recipes`, `get_user_inventory`)를 호출하고, 그 결과를 반영한 최종 응답을 생성하도록 한다.
...
3. **GPT Function Call (선택적)**: OpenAI Chat Completions API에 **tools**(도메인 함수 정의)를 넘기고 스트리밍 호출한다. `tool_choice: 'auto'` 를 사용하여 **모든 tool call 여부·횟수는 GPT가 판단**하게 한다. 예를 들어,
   ...
   - 유저 보유 재료가 필요하면 `get_user_inventory({ userId })` 를 호출하여 유저 재료 컨텍스트를 확보한다.
   - 레시피 검색이 필요하다고 판단하면 `search_recipes` 를 호출한다. 이때 사용자 발화에서 해석한 조건(키워드, 조리시간/인분 범위, 포함/제외 재료명, 카테고리 id 등)은 **반드시 `search_recipes` 인자에 명시적으로 전달**한다. 예: `search_recipes({ keywords: ['간단','저녁'], cookTime: { lte: 30 }, avoidIngredients: ['우유'] })`.
4. **도메인 Tool Handlers**:
   - **InventoryHandler (`get_user_inventory`)**: MongoDB의 `Inventory` 문서에서 `ingredients.owned`, `ingredients.favorite`, `recipes.favorite` 컨텍스트를 조회한 뒤, 재료 ID는 **PostgreSQL(Prisma) `Ingredient` 마스터와 조인**하여 `[{ id, name, ... }]` 정보를 보강해 반환한다. 이때, **재료 마스터는 Redis 등으로 적절히 캐싱**하여 id → name 변환을 반복적으로 수행하지 않도록 한다.
   - **SearchRecipesHandler (`search_recipes`)**: Function call 인자를 받아 **semantic-first** 파이프라인으로 레시피 후보를 수집한다. (1) 구조화 질의 텍스트 구성 → (2) LLM Query Expansion → (3) pgvector ANN top-K(`RecipeEmbeddingRepository.searchTopK`) → (4) hard constraint(`isPublished`, `avoidIngredientIds`/`avoidIngredients`) 적용 후 Prisma 상세 조회 → (5) semantic·keyword·inventory·user preference·soft constraint 가중 재랭킹. `cookTime`/`servings`/카테고리/`mustHaveIngredients`는 **soft signal**(탈락이 아닌 점수화)로 처리한다. `ingredientIds`는 보유 재료 매칭 가산점에 사용한다. 유저 재료 기반 추천이 필요하면 GPT가 먼저 `get_user_inventory`를 호출해 id 배열을 `search_recipes`에 전달하도록 유도한다. 검색 결과는 `SearchedRecipe[]`(최대 10건, `reasonSignals`·점수 포함) 형태로 GPT에 반환한다. 튜닝 상수는 `server/consumer/src/policy/recipe-search.policy.ts`, 상세 명세는 `../spec/backend_architecture_spec_consumer.md` §2.7.
...
- **함수 정의 (OpenAI tools)**: OpenAI `tools` 배열에 `search_recipes`, `get_user_inventory`, `get_food_categories`, `finalize_recipe_selection` 등 도메인 함수의 이름·설명·parameters(JSON Schema)를 명시한다.
  - `get_user_inventory` 예시: `parameters: { type: 'object', properties: { userId: { type: 'string' } }, required: ['userId'] }`
  - `search_recipes` 예시: `parameters: { type: 'object', properties: { keywords: { type: 'array', items: { type: 'string' } }, ingredientIds: { type: 'array', items: { type: 'number' } }, mustHaveIngredients: { type: 'array', items: { type: 'string' } }, avoidIngredientIds: { type: 'array', items: { type: 'number' } }, avoidIngredients: { type: 'array', items: { type: 'string' } }, cookTime: { type: 'object', properties: { gte: { type: 'number' }, lte: { type: 'number' } } }, servings: { type: 'object', properties: { gte: { type: 'number' }, lte: { type: 'number' } } }, recipeCategoryIds: { type: 'array', items: { type: 'number' } }, ingredientCategoryIds: { type: 'array', items: { type: 'number' } } } }` 형태로 확장 필드를 함께 정의해, 파싱된 조건이 숨은 서버 상태 없이 tool 인자만으로 재현되게 한다. 구현 SSOT는 `chatbot-tools.definition.ts`.
...
- **유저 재료 컨텍스트 처리**: 유저 재료 스키마에는 재료 ID만 저장되므로, **id → name 변환은 항상 `get_user_inventory` 핸들러 내부에서 수행**한다. 이때 Ingredient 마스터를 캐시(예: Redis key `ingredient:by-id:{id}`) 하여 반복 조회를 줄인다. GPT는 이 tool 결과로부터 유저가 보유한 재료명을 이해하고, 이후 대화나 추가 tool call(`search_recipes`)에 활용한다.
- **검색·매칭**: `SearchRecipesHandler`는 pgvector ANN으로 의미 기반 후보를 넓게 수집한 뒤, hard constraint(기피 재료·published)를 적용하고 composite score로 재랭킹해 상위 N건을 `SearchedRecipe[]`로 반환한다. 유저 재료 컨텍스트는 `get_user_inventory` tool 결과의 `ingredientIds`로 `inventoryMatchScore`에 반영되며, **Producer가 Kafka 이벤트에 owned/favorites 배열을 직접 실어 나르지 않는다.**
...
- 추천 점수 갱신은 Producer 동기 API가 아닌 Kafka Consumer 경로(§3 Event Sourcing)로 수행한다.
- 캐시 무효화는 Handler가 Kafka를 직접 발행하지 않고 `CacheInvalidationRequestService`만 호출한다(§4.1).
...
- **@mealio/shared** 는 환경 변수를 직접 검증하지 않는다. `createKafkaConfig`, `createRedisConfig`, `mongooseConfig` 등은 **`process.env.VAR!`** 로 읽어 설정 객체를 반환하며, 실제 검증은 이를 사용하는 Producer/Consumer의 `config/env.validation.ts` 에서 앱 기동 시 수행한다. 즉, 시스템 전체의 “모든 환경 변수는 기본적으로 필수”라는 원칙은 **각 애플리케이션의 validation 스키마를 통해 강제**되며, shared 패키지는 단독으로 앱 구동을 중단시키지 않는다.

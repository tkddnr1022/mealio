...
| 2 | LLM Query Expansion — Responses Structured Outputs(`json_schema`, `recipe_search_query_expansion`, 모델 `OPENAI_QUERY_EXPANSION_MODEL`) (실패 시 원질의 fallback) |
...
| 이벤트 | 동작 |
|--------|------|
| `chatbot.start` | LLM(`OPENAI_TITLE_MODEL`)으로 `title`·메타 생성 (`createWithTitle`, `titleSource: llm`) |

...
| persist | Kafka `recipe-ingestion-retrieved` | — | payload `{ startSourceId, endSourceId, fetchedCount, triggeredAt }` |
...
1. fetch 성공·`fetchedCount > 0`일 때 `recipe-ingestion-fetch-completed` 발행 — payload `{ startSourceId, endSourceId, fetchedCount, triggeredAt }`, key = `{startSourceId}:{endSourceId}`
2. `INFO-200`(0건) 또는 upsert 실패만 있는 경우 트리거 미발행
3. submit consumer는 payload를 트리거 신호로만 사용하고, 실제 제출 대상은 Mongo `status: fetched`를 재조회해 결정
4. cron fallback submit CLI는 `--start-source-id`/`--end-source-id` 미지정 시 전체 `fetched` backlog에서 `submitBatchSize`만큼 처리
...
1. consume 시 payload `{ startSourceId, endSourceId, fetchedCount, triggeredAt }`를 트리거 신호로 수신
2. Mongo `recipe_ingestion_jobs`에서 `status: retrieved` + `source_id` 구간(`startSourceId`~`endSourceId`) 후보를 재조회
...
`status: fetched` 항목 중 최대 `submitBatchSize`건을 OpenAI Batch API에 제출한다. `startSourceId`/`endSourceId`가 지정되면 `source_id`(RCP_SEQ) 필터를 적용한다. `startSourceId`만 지정 시 `endSourceId = startSourceId + submitBatchSize - 1`, `endSourceId`만 지정 시 `source_id <= endSourceId`(max index)로 조회한다. 가용 건수가 `submitBatchSize` 미만이면 가용 건만 제출하고 종료한다.

1. **대상 조회 및 상태 전환**
   ```
   status: fetched job 최대 submitBatchSize건 조회
   (startSourceId/endSourceId 지정 시 source_id 필터: 하한 $gte startSourceId, 상한 $lte endSourceId — 각각 생략 가능)
   (0건이면 no-op 종료)
...
4. **retrieve 완료 트리거 발행**
   ```
   batch 처리 전체에서 성공한 job의 source_id min/max, 성공 건수 집계
   → Kafka recipe-ingestion-retrieved
      payload: { startSourceId, endSourceId, fetchedCount, triggeredAt }
      key: "{startSourceId}:{endSourceId}"
   ```

### 5.4 persist

Kafka `recipe-ingestion-retrieved` 소비 → payload source 구간으로 `retrieved` job 재조회 → PostgreSQL 영속화. 멱등성은 §2.5.
...
| `startSourceId` / `endSourceId` | `--start-source-id` / `--end-source-id` | submit | — (미지정 시 전체 backlog; `--start-source-id`만이면 `submitBatchSize`로 end 계산; `--end-source-id`만이면 max index 상한) |

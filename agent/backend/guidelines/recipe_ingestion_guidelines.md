...
| `status` | `fetched` \| `submitting` \| `parse_submitted` \| `retrieving` \| `parse_retrieved` \| `persisting` \| `persisted` \| `failed` |
| `retry_count` | 최대 3회 |
| `run_id` | fetch 시작 시 생성되는 파이프라인 실행 단위 UUID |
| `raw_data` | JSON (공공데이터 API row 원본) |
| `batch_id` | OpenAI Batch Job ID (`parse_submitted` 이후) |
...
3. **결과 파싱 및 job 반영** — output JSONL 라인 단위
   ...
   error != null 또는 status_code != 200
     ...
     → retrieved_data, status: parse_retrieved
   ```
...
```
fetched
  → (submit) → submitting → parse_submitted
  → (retrieve) → retrieving → parse_retrieved

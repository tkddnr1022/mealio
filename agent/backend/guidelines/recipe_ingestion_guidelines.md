...
| 계층 | 저장소 | 역할 |
|------|--------|------|
| **Ingestion 파이프라인** | MongoDB `recipe_ingestion_jobs` | 단계별 `status`, `run_id`, `raw_data`, `retrieved_data`, `parse_batch_id`, `embed_batch_id`, 재시도·에러의 **유일한 진실 원천** |
...
| parse-submit | CLI (standalone job) + Kafka `recipe-ingestion-parse-submit-triggered` | `job:recipe-ingestion-parse-submit` | OpenAI Parse Batch 제출. Kafka 트리거는 신호만, 대상은 `status: fetched` 조회 |
| parse-retrieve | CLI (standalone job) | `job:recipe-ingestion-parse-retrieve` | Parse Batch 완료 확인·결과 반영 |
| persist | Kafka `recipe-ingestion-persist-triggered` | `job:recipe-ingestion-persist` | payload `{ runId, fetchedCount, triggeredAt }` |
| embed-submit | CLI (standalone job) + Kafka `recipe-ingestion-embed-submit-triggered` | `job:recipe-ingestion-embed-submit` | OpenAI Embedding Batch 제출 |
...
pnpm --filter consumer run job:recipe-ingestion-parse-retrieve --run-id <runId>
pnpm --filter consumer run job:recipe-ingestion-persist --run-id <runId>
pnpm --filter consumer run job:recipe-ingestion-embed-submit --run-id <runId>
pnpm --filter consumer run job:recipe-ingestion-embed-retrieve --run-id <runId>
...
- submit cron과 Kafka 트리거가 동시에 실행되어도 `fetched → parse_submitting` 조건부 전환으로 동일 job 중복 Batch 제출을 방지 (persist와 동일 패턴)
...
| submit | standalone job + consumer | cron fallback + Kafka 구독 | `server/consumer/src/jobs/recipe-ingestion-parse-submit/`, `server/consumer/src/consumers/recipe-ingestion-parse-submit/` |
| retrieve | standalone job | cron → CLI | `server/consumer/src/jobs/recipe-ingestion-parse-retrieve/` |
...
cron ──→ job:recipe-ingestion-fetch ──→ MongoDB (recipe_ingestion_jobs, status: fetched)
                                      └──→ Kafka (recipe-ingestion-parse-submit-triggered)
Kafka ──→ recipe-ingestion-parse-submit consumer ──→ MongoDB ──→ OpenAI Batch API (parse)
cron ──→ job:recipe-ingestion-parse-submit ──→ MongoDB ──→ OpenAI Batch API (parse fallback)
cron ──→ job:recipe-ingestion-parse-retrieve ──→ MongoDB ──→ Kafka (recipe-ingestion-persist-triggered)
cron/manual ──→ job:recipe-ingestion-persist ──→ MongoDB + PostgreSQL (Recipe domain) ──→ Kafka (recipe-ingestion-embed-submit-triggered)
Kafka ──→ recipe-ingestion-embed-submit consumer ──→ MongoDB ──→ OpenAI Batch API (embedding)
cron ──→ job:recipe-ingestion-embed-submit ──→ MongoDB ──→ OpenAI Batch API (embedding fallback)
cron ──→ job:recipe-ingestion-embed-retrieve ──→ MongoDB + PostgreSQL(RecipeEmbedding)
```
...
1. fetch 성공·`fetchedCount > 0`일 때 `recipe-ingestion-parse-submit-triggered` 발행 — payload `{ runId, fetchedCount, triggeredAt }`, key = `runId`
...
5. `fetched → parse_submitting` 조건부 전환으로 cron·Kafka 트리거 중복 실행 시 동일 job 중복 제출을 방지 (`lockedCount === 0`이면 no-op)
6. Kafka redelivery·실패 시 DLQ `recipe-ingestion-parse-submit-triggered-dlq` 위임
...
2. Mongo `recipe_ingestion_jobs`에서 `status: parse_retrieved` + `runId` 후보를 재조회
3. `status === parse_retrieved`일 때만 `status: persisting`으로 **조건부 전환**(낙관적 락). 이미 `persisting` / `persisted`이면 skip
...
6. 실패 시 `retry_count++`, `status: parse_retrieved`로 되돌려 Kafka 재처리 또는 DLQ(`recipe-ingestion-persist-triggered-dlq`) 위임
...
MongoDB 컬렉션 (Mongoose). 파이프라인 SSOT. Mongoose 프로퍼티는 camelCase, 아래 표는 Mongo 필드명(snake_case) 기준이다.

| 필드 | 설명 |
|------|------|
| `source_id` | 공공데이터 원본 ID — API 응답 `RCP_SEQ` (Number, unique) |
| `status` | §3.1.1 enum |
| `retry_count` | 최대 3회 (`MAX_RECIPE_INGESTION_RETRY_COUNT`) |
| `run_id` | fetch 시작 시 생성되는 파이프라인 실행 단위 UUID |
| `raw_data` | JSON (공공데이터 API row 원본) |
| `parse_batch_id` | OpenAI Parse Batch Job ID (`parse_submitted` 이후) |
| `embed_batch_id` | OpenAI Embedding Batch Job ID (`embed_submitted` 이후) |
| `retrieved_data` | JSON (parse Batch LLM 변환 결과) |
...
| `parse_retrieved` | parse-retrieve | LLM `retrieved_data` 반영 완료, persist 대기 |
...
| `failed` | — | `retry_count` 상한 초과 등으로 중단, 수동 검수·재큐잉 대상 |
...
Mongoose: `parseBatchId`, `embedBatchId`.

| 필드 | 설정 시점 | 비고 |
|------|-----------|------|
| `parse_batch_id` | `parse_submitted` 전환 시 | parse Batch API 응답 ID. retrieve·롤백 시 `parse_batch_id`·`parse_submitted_at` 초기화 |
| `embed_batch_id` | `embed_submitted` 전환 시 | embed Batch API 응답 ID. retrieve·롤백 시 `embed_batch_id`·`embed_submitted_at` 초기화 |

parse·embed 단계는 각각 독립 Batch ID를 유지한다. `run_id`와 `parse_batch_id`는 1:1 불변식을 따른다.
...
- **단계 시작**: 각 단계 시작 시 `status: {단계}ing` — `parse_submitting`, `parse_retrieving`, `persisting`, `embed_submitting`, `embed_retrieving`
- **단계 실패**: `retry_count++`, `status: {재시도 필요 단계}`; `retry_count >= MAX_RETRY_COUNT`이면 `status: failed`, `failed_at` 기록
...
| `RCP_PAT2` | — (출력 필드 없음) | — | **카테고리 선택 힌트만** — LLM이 `recipe.categoryId` / `proposedCategory` 결정 시 참조. DB·API에 저장하지 않음 |
...
submit은 선택된 job 그룹을 Batch API에 제출하고, 반환된 OpenAI Batch ID를 각 job의 `parse_batch_id`에 기록한다.
...
2. **runId 그룹별 상태 전환**
   ```
   status: fetched → status: parse_submitting (일괄 update)
   ```
...
4. **JSONL 생성** — runId 그룹별 대상 job, `custom_id` = `recipe_ingestion_job._id`
   ...
   - **이미지·영양·조리 메타**: `imageUrl`, `nutrition`, `cookingMethod`, `steps[].imageUrl`
   - **`RCP_PAT2`**: 출력·저장 대상이 아님. `recipe.categoryId` / `proposedCategory` 선택의 힌트로만 사용
   - 어조 규칙 (~요 체 등)
...
`status: parse_submitted` job의 OpenAI Batch 완료를 확인하고, 완료 batch output을 Mongo에 반영한 뒤 Kafka로 persist를 트리거한다. **cron → CLI** (`job:recipe-ingestion-parse-retrieve`) 1회 실행 단위.

1. **대상 batch 조회 및 Batch API 상태 확인**
   ```
   status: parse_submitted인 distinct parse_batch_id 조회
     for each parse_batch_id:
       GET https://api.openai.com/v1/batches/{parse_batch_id}

       completed → 해당 batch job status: parse_retrieving 후 2~3단계
...
4. **retrieve 완료 트리거 발행**
   ...
   → Kafka recipe-ingestion-persist-triggered
...
Kafka `recipe-ingestion-persist-triggered` 소비 → payload `runId`로 `parse_retrieved` job 재조회 → PostgreSQL 영속화. 멱등성은 §2.5.

- 실패: `retry_count++`, `status: parse_retrieved` 복귀 → Kafka 재처리; 반복 실패 시 DLQ `recipe-ingestion-persist-triggered-dlq` (`@mealio/shared` `KAFKA_TOPICS`)
...
4. Recipe + RecipeIngredient **transaction upsert** — `(source, sourceRecipeId)` unique; `parse_confidence`가 `RECIPE_INGESTION_MIN_PUBLISH_PARSE_CONFIDENCE`(`high`) 미만 → `isPublished: false`. persist 허용 하한은 `RECIPE_INGESTION_MIN_PARSE_CONFIDENCE`(`low`, `@mealio/shared` policy SSOT)
   - **이미지·영양·조리 메타**: 검증된 `retrieved_data` → `imageUrl`, `nutrition`, `cookingMethod`, `cookingTip`, `instructions[].imageUrl` (§4.5)
...
`status: persisted` job을 OpenAI Embedding Batch API에 제출한다. parse-submit과 동일하게 Kafka 트리거·cron fallback·`persisted → embed_submitting` 조건부 락 패턴을 따른다.
...
4. **Batch 제출** 후 `embed_submitting → embed_submitted`, `embed_batch_id`, `embed_submitted_at` 기록
5. 실패 시 `retry_count++`, `status: persisted` 복귀 또는 `failed`

### 5.6 embed-retrieve

`status: embed_submitted` job의 Embedding Batch 완료를 확인하고 RecipeEmbedding·IngredientEmbedding을 upsert한다.

1. **대상 batch 조회** — `status: embed_submitted` distinct `embed_batch_id`
...
4. `failed`/`expired`·라인 오류: `retry_count++`, `status: persisted` 복귀 또는 `failed` (`embed_batch_id`·`embed_submitted_at` 초기화)
...
[always-on] submit consumer → OpenAI Batch → Mongo (parse_submitted)
[cron] submit CLI → OpenAI Batch → Mongo (parse_submitted)  # fallback
[cron] retrieve CLI → Mongo (parse_retrieved) + Kafka emit
[always-on] persist consumer → PostgreSQL + Kafka embed-submit emit
[always-on] embed-submit consumer → OpenAI Embedding Batch → Mongo (embed_submitted)
[cron] embed-submit CLI → Mongo (embed_submitted)  # fallback
[cron] embed-retrieve CLI → PostgreSQL (RecipeEmbedding)
```
...
| `parse_confidence: low`·`medium` 비율 | 품질 |
...
구현: `server/consumer/src/reliability/monitoring/consumer-metrics.service.ts`

| 메트릭 | 라벨 | 기록 시점 |
|--------|------|-----------|
| `recipe_ingestion_stage_total` | `stage`, `outcome` | 각 stage service (success / failed / skipped) |
| `recipe_ingestion_stage_latency_ms` | `stage` | stage 완료 시 |
| `recipe_ingestion_parse_confidence_total` | `level` | persist 성공 시 |
| `recipe_ingestion_ingredient_match_total` | `match_method` | persist 성공 시 |
| `recipe_ingestion_llm_tokens_total` | `token_type` | parse-retrieve row 성공 시 |

**수집 경로**

| 실행 주체 | 노출 방식 |
|-----------|-----------|
| always-on Consumer | `METRICS_PORT` `/metrics` (MonitoringModule) |
| standalone CLI (`run-*.ts`) | 종료 직전 Pushgateway push (`metrics-push.ts`) |

CLI는 단명 프로세스이므로 in-memory registry를 Pushgateway로 push한다. `PUSHGATEWAY_URL`이 설정되어 있고 `METRICS_ENABLED=true`일 때만 push하며, push 실패는 warn 로그만 남기고 job 종료 코드에는 영향을 주지 않는다.
...
Push grouping: `job=recipe_ingestion_cli`, `stage=<fetch|parse-submit|…>`. Prometheus는 `docker/pushgateway`의 Pushgateway를 `honor_labels: true`로 scrape한다.

### 7.1 구조화 로깅 계약

recipe-ingestion 단계별 service·CLI는 `@mealio/shared` `logStructured`를 **`recipe-ingestion-logger.ts`** 래퍼로 호출한다. Kafka consumer(`BaseTopicProcessor`)·Producer HTTP 미들웨어와 동일하게 JSON 한 줄 출력이다.

| 필드 | 설명 |
|------|------|
| `event` | 고정 이벤트명 (`recipe_ingestion_stage_started`, `recipe_ingestion_batch_submitted` 등) |
| `stage` | `fetch` \| `parse-submit` \| `parse-retrieve` \| `persist` \| `embed-submit` \| `embed-retrieve` |
| `correlationId` | CLI 실행마다 생성. Kafka 트리거 경로는 메시지 헤더에서 전달 |
| `runId` / `jobId` / `batchId` | 단건·배치 추적 식별자. Mongo 필드는 `parseBatchId`·`embedBatchId` (로그 필드명은 공통 `batchId`) |
| `outcome` | `success` \| `skipped` \| `failed` \| `no_op` |
| `durationMs` | 단계 완료 시 소요 시간 |

**레이어 책임**

| 레이어 | 로그 범위 |
|--------|-----------|
| CLI (`run-*.ts`) | `recipe_ingestion_cli_started` / `cli_completed`, scope 옵션·집계 결과 |
| Service | 단계 lifecycle, batch submit/retrieve, Kafka trigger 실패, skip 집계 |
| Domain | 비즈니스 의미 이벤트(신규 재료·dedup 등). per-job 성공은 `debug` |

**금지**: `rawData`, `retrievedData`, embedding vector, OpenAI 응답 본문. 식별자·`status_code`·`errorMessage`만 기록한다.

구현: `server/consumer/src/jobs/recipe-ingestion/recipe-ingestion-logger.ts`
...
```
fetched
  → (parse-submit) → parse_submitting → parse_submitted
  → (parse-retrieve) → parse_retrieving → parse_retrieved
  → (persist) → persisting → persisted
  → (embed-submit) → embed_submitting → embed_submitted
  → (embed-retrieve) → embed_retrieving → embed_retrieved
  → (retry_count >= 3) → failed
```

재시도 시 이전 안정 단계로 복귀: parse 단계 → `fetched`, persist → `parse_retrieved`, embed → `persisted`.

`failed`는 최종 상태이며 수동 검수·`requeueFailedToFetched` 재큐잉 대상이다.

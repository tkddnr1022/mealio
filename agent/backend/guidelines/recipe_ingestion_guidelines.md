...
| `batch_id` | OpenAI Batch Job ID (`parse_submitted`·`embed_submitted` 이후) |
...
submit은 선택된 job 그룹을 Batch API에 제출하고, 반환된 `batch_id`를 각 job에 기록한다.
...
1. **대상 batch 조회 및 Batch API 상태 확인**
   ```
   status: parse_submitted인 distinct batch_id 조회
     for each batch_id:
...
| `runId` / `jobId` / `batchId` | 단건·배치 추적 식별자 |

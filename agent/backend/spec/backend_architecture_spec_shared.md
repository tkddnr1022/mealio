...
| Recipe | id, categoryId(@map category), title, description?, instructions(Json), difficulty, cookTime, imageUrl?, servings, cookingMethod?, dishType?, nutrition(Json)?, cookingTip?, source?, sourceRecipeId?, isPublished, createdAt, updatedAt | categoryMeta(RecipeCategory), recipeIngredients, stats(RecipeStats, 1:1 optional) 관계. `instructions` JSON: `{ step, content, imageUrl? }`. `nutrition` JSON: `{ calories, carbohydrates, protein, fat, sodium }`. @@index(categoryId, difficulty, cookTime, createdAt), @@index(difficulty, cookTime, createdAt), (createdAt Desc), @@unique(source, sourceRecipeId) |
...
| server/shared/src/config/observability.config.ts | METRICS_ENABLED·`SLOW_QUERY_THRESHOLD_MS`·Sentry DSN·`PUSHGATEWAY_URL`(optional) 등 env 기반 관측성 설정 |

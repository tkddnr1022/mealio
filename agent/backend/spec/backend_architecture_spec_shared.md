...
| Recipe | id, categoryId(@map category), title, description?, instructions(Json), difficulty, cookTime, imageUrl?, servings, cookingMethod?, nutrition(Json)?, cookingTip?, source?, sourceRecipeId?, isPublished, createdAt, updatedAt | categoryMeta(RecipeCategory), recipeIngredients, stats(RecipeStats, 1:1 optional) 관계. `instructions` JSON: `{ step, content, imageUrl? }`. `nutrition` JSON: `{ calories, carbohydrates, protein, fat, sodium }`. @@index(categoryId, difficulty, cookTime, createdAt), @@index(difficulty, cookTime, createdAt), (createdAt Desc), @@unique(source, sourceRecipeId) |

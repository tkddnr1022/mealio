...
| 잡 ID | 주기 | 대상 | 저장 | 비고 |
|-------|------|------|------|------|
| `rollup_eventlog_daily` | 일 1회 | EventLog KPI A,C + search CTR | `kpi_rollups` 컬렉션 또는 BI DB | EC2 cron (`docker/scripts/consumer.cron.example`) |
...
docker/scripts/run-job.sh kpi-rollup --backfill 7
```

실행 스크립트: `docker/scripts/run-job.sh` (consumer 이미지를 재사용한 일회성 컨테이너).

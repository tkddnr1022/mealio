...
type ButtonOwnHtmlKeys = "className" | "children" | "variant" | "size" | "label" | "aria-label";
...
function a11yNameFromContent(label: string | undefined, children: ReactNode): string {
...
function buttonShowsOnlyPlainText(label: string | undefined, children: ReactNode): boolean {
  if (children == null || children === false || children === true) return Boolean(label?.trim());
...
export function Button(props: ButtonProps) {
  ...
  const disabledNative = isButtonAsLink(props) ? !!props.disabled : !!props.disabled;
  ...
  const ariaLabelForButton =
    !buttonShowsOnlyPlainText(label, children) ? buildAriaLabel("button", a11yName) : undefined;
  ...
  const surfaceClassName = cn(baseClassName, sizeClassName, toneClassName, className);
  ...
  if (isButtonAsLink(props)) {
    ...
    if (isInternalNavHref(href)) {
      return (
        <Link href={href} className={linkClassName} {...anchorRest} aria-label={ariaLabelForLink}>
          {content}
        </Link>
      );
    }
    ...
    return (
      <a href={href} className={linkClassName} {...anchorRest} aria-label={ariaLabelForLink}>
        {content}
      </a>
    );
  }
  ...
  return (
    <button
      ...
      {...buttonRest}
      {...(ariaLabelForButton !== undefined ? { "aria-label": ariaLabelForButton } : {})}
    >
      {content}
    </button>
  );

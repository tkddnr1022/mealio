  ...
  return (
    <LegalDocumentView title="이용약관" effectiveDate={EFFECTIVE_DATE}>
      ...
      <LegalSection title="3. 계정 및 데이터">
        <p className="m-0">
          계정은 Google·Kakao·Naver 등 소셜 로그인으로 생성될 수 있습니다. 데모
          특성상 계정·보관함·대화·활동 기록 등은 점검·재배포·용량 관리 등의
          이유로{' '}
      ...
      <LegalSection title="4. 이용 시 유의사항">
        <ul className="m-0 list-disc space-y-1 pl-5">
          ...
          <li>
            레시피·영양·챗봇 응답은 참고용이며, 건강·알레르기·안전에 대한 전문
            조언을 대체하지 않습니다.
          </li>
          <li>
            공공데이터·제3자 API·LLM 응답의 정확성·연속성을 보장하지 않습니다.
          </li>
          <li>데모 크레딧은 체험용이며 환불·양도·금전적 가치가 없습니다.</li>
        </ul>
      </LegalSection>
      ...
      <LegalSection title="5. 책임의 제한">
        <p className="m-0">
          ...
          장애로 인한 손해에 대해, 관련 법령이 허용하는 범위에서 운영자·기여자는
          책임을 지지 않습니다. MIT 라이선스의 면책 조항이 함께 적용됩니다.
        </p>
      </LegalSection>

      <LegalSection title="6. 오픈소스 및 콘텐츠">
        <p className="m-0">
          저장소 코드의 이용·복제·수정·배포는 MIT 라이선스를 따릅니다. 레시피 등
          일부 데이터는 공공데이터·외부 출처에 기반할 수 있으며, 해당 출처의
          이용 조건을 함께 확인하세요. 서비스 UI에 표시되는 브랜드명·디자인은
          데모 식별을 위한 것이며, 상업적 오해를 유도하지 않도록 사용해 주세요.
        </p>
      </LegalSection>

      <LegalSection title="7. 약관 변경">
        <p className="m-0">
          데모 운영에 맞춰 본 문서를 수시로 갱신할 수 있습니다. 변경 내용은 본
          페이지에 게시된 시점부터 적용됩니다.
        </p>
      </LegalSection>

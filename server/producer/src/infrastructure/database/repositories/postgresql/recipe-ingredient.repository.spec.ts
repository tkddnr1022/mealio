...
describe('RecipeIngredientRepository', () => {
  ...
  describe('createMany', () => {
    it('should create many recipe ingredients', async () => {
        ...
        expect(prisma.recipeIngredient.createMany).toHaveBeenCalledWith({ data: createInput });
  ...
  describe('update', () => {
    it('should update a recipe ingredient', async () => {
        ...
        expect(prisma.recipeIngredient.update).toHaveBeenCalledWith({ where: { id: 1n }, data: updateInput });
        expect(result).toEqual(mockRecipeIngredient);
    });
  });
    ...
    it('should delete recipe ingredients by recipe id', async () => {
        ...
        expect(prisma.recipeIngredient.deleteMany).toHaveBeenCalledWith({ where: { recipeId: 1n } });
        expect(result).toEqual({ count: 1 });
    });
    });
});

...
import { createKafkaConfig, LOCAL_TOPIC_CONFIG } from 'src/shared/configs/kafka.config';
...
export class KafkaAdminService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    if (nodeEnv === 'production') {
      this.logger.log('Skipping Kafka topic creation in production environment');
      return;
    }
    ...
    try {
      await admin.connect();
      this.logger.log(`Kafka admin connected (clientId=${kafkaConfig.clientId}, brokers=${brokers.join(',')})`);
      ...
      if (topicsToCreate.length === 0) {
        this.logger.log('All required topics already exist. No topics to create.');
        return;
      }

      // 존재하지 않는 토픽만 생성
      this.logger.log(`Creating Kafka topics: ${topicsToCreate.map((t) => t.topic).join(', ')}`);
      ...
      try {
        ...
        this.logger.error('Error while disconnecting Kafka admin', error as Error);
      }
    }
  }
}

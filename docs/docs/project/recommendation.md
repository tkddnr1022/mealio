...
sequenceDiagram
    ...
    alt 캐시 hit
        ...
        P->>PG: UserRecipeRecommendation SSOT 조회
        P->>R: TTL 3600s 저장
    end
...
- Producer API는 이 SSOT를 조회해 응답 (캐시 miss 시)

SSOT에 데이터가 없으면 **인기 레시피(`likeCount` DESC)** 로 fallback합니다.
...
전체 표: `agent/backend/spec/backend_architecture_spec_consumer.md` §2.6.2
...
다음 조회 시 DB 폴백으로 최신 SSOT를 반영합니다.

## 운영·KPI

- **E2E 지연 KPI**: `kpi_recommendation_e2e_latency` — `recipe.favorites_add` 발생부터 추천 반영까지
- 지연 알림: `agent/observability/product_kpi_runbook.md` §2.3

## 변경 시 체크리스트

1. 가중치 변경 → `recommendation-score.service.ts` + 명세 §2.6.2
...
- `agent/backend/spec/backend_architecture_spec_shared.md` (§3.5)

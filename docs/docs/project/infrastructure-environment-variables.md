...
`compose-database.yml`에서 root 계정을 설정합니다.
...
`compose-database.yml`에서 초기 DB·계정을 설정합니다.
...
`compose-kafka.yml`에서 브로커 외부 리스너를 설정합니다.
...
| Compose | `compose-kafka-ui.yml` |
| 패턴 | `KAFKA_UI_BOOTSTRAP_SERVERS`는 Docker 네트워크 내부 주소(`kafka:19092`)를 사용 |

## Prometheus

`compose-monitoring.yml`에서 스크랩 대상 모드를 선택합니다.
...
`compose-monitoring.yml`에서 대시보드·알림을 설정합니다.

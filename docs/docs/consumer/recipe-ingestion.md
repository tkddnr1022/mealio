...
| Mongo | Mongoose | 설정 시점 |
| --- | --- | --- |
| `parse_batch_id` | `parseBatchId` | parse-submit → `parse_submitted` |
| `embed_batch_id` | `embedBatchId` | embed-submit → `embed_submitted` |
...
| 2 | parse-submit | `parse_submitted` 증가, `parseBatchId` 기록 |
...
| 5 | embed-submit | `embed_submitted` 증가, `embedBatchId` 기록 |

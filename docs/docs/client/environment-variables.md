...
기본 포트 **4000**. 검증·파싱: `client/src/.../env.ts` — 실패 시 `env.validationErrors`에 수집, `assertEnv()`로 엄격 검증 가능.
...
| 예시 | `local` |
| 사용처 | `client/src/.../env.ts` → `env.runtime`, Sentry 샘플링 분기 |
...
| 사용처 | `resolveApiBaseUrl()` (브라우저), `client/src/.../http-client.ts`, OAuth 진입 URL (`client/src/.../providers.ts`) |
...
| 사용처 | `resolveApiBaseUrl()` (서버), `client/src/.../route.ts` |
...
| 예시 | `/api/v1` |
| 기본값 | `/api/v1` |
| 사용처 | `client/src/.../endpoints.ts` → `API_PREFIX` |
...
| 사용처 | `client/src/.../route.ts` — `timingSafeEqual` 비교 |
...
| 예시 | `G-XXXXXXXXXX` |
| 사용처 | `client/src/.../layout.tsx`, `ObservabilityBootstrap.tsx` |
...
| 사용처 | `client/src/.../sentry.config.ts` |

...
- Figma 디자인을 코드로 옮길 때 무엇을 SSOT로 봐야 하는가?
- 디자인 토큰 → CSS 워크플로우는?
- 구현 시 금지·권장 사항은?
...
| 순서 | 파일 | 역할 |
| --- | --- | --- |
| 1 | `agent/design/spec/design_tokens.json` | 컬러·타이포·스페이싱·컴포넌트 토큰 |
| 2 | `client/src/app/globals.css` | `:root`, `@theme`, 컴포넌트 recipe |
| 3 | `agent/design/spec/design_principle.json` | UX·a11y·성능·톤 원칙 |
...
| 단계 | 가이드 |
| --- | --- |
| 1. 수집 | `agent/design/guidelines/design_system_collection_guidelines.md` |
...
- 피드백: `agent/design/temp/` (예: `figma-variables-and-styles-feedback.md`)
...
- `agent/design/guidelines/design_to_code_guidelines.md`

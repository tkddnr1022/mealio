...
| **추천 SSOT** | `UserRecipeRecommendation` 테이블 — 개인화 추천 결과 원본 |
...
`kpi_recommendation_e2e_latency` — favorites_add 이후 SSOT·캐시 반영까지 지연. `user-events` lag·RecommendationHandler 확인.
...
**명세 또는 코드 중 하나가 잘못된 것** — 스펙 주도 개발 원칙에 따라 동기화 PR을 올립니다.

### Docusaurus와 `agent/` 폴더 관계?

- `agent/` — SSOT 명세·가이드 (에이전트·개발자용 원본)
- `docs/` — Docusaurus 탐색·온보딩용 (링크·요약)

계획: `agent/docusaurus_documentation_plan.md`

### 새 API를 추가할 때?

1. OpenAPI YAML
2. backend 명세 §1.1
3. (필요 시) event_dictionary
...
| API 목록 | [데이터/계약 인덱스](../project/contracts-index) |
| 기여 | [기여 가이드](./contributing) |

## SSOT

- `agent/common/proposal.md` — 서비스 개요
- `agent/common/schema.md` — 데이터 의미

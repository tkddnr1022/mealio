...
**문서 또는 코드 중 하나가 잘못된 것** — [기여 가이드](./contributing)의 문서·코드 정합성 원칙에 따라 동기화 PR을 올립니다.

### 공개 문서 사이트는 무엇을 다루나요?

아키텍처·운영·온보딩 **요약과 교차 링크**입니다. 필드 단위 API·전체 파일 트리 등 구현 세부는 내부 명세·OpenAPI를 따릅니다.

→ [프로젝트 개요](../)

### 새 API를 추가할 때?

1. `server/producer/.../modules/` (Swagger DTO)
2. [Producer 도메인 API](../producer/domain-api)
3. (필요 시) [Observability](./observability) 이벤트 사전 등록
4. 본 문서 사이트 해당 페이지 갱신

...
예외 작업을 수행한 경우에는 **반드시 내부 명세·Docusaurus 문서를 동기화**합니다. 세부 규칙은 [개발 규약](./development-conventions)을 참고합니다.
...
코드 변경과 문서 갱신은 **같은 PR**에서 함께 리뷰받습니다.
...
- 새 페이지는 「이 문서로 해결할 질문」과 「관련 문서」를 포함하고, 코드·계약 링크를 본문에 연결합니다.
- 스텁에서 완성 문서로 전환할 때는 주제별 본문 섹션 2개 이상, 코드 경로 표 또는 Mermaid 1개 이상을 채웁니다.

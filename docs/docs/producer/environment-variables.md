...
| 예시 | `500` |
| 패턴 | `METRICS_ENABLED=true`일 때 필수. 양의 정수 |
| 사용처 | `server/producer/.../slow-query.interceptor.ts`, `@mealio/shared` `observability.config.ts` |
...
| 항목 | 내용 |
| --- | --- |
| 설명 | Next.js SSR·빌드 등 내부 API 트래픽의 별도 레이트 리밋 인증 시크릿 |
| 예시 | 임의의 긴 랜덤 문자열 (client `INTERNAL_API_SECRET`과 동일) |
| 사용처 | `server/producer/.../rate-limit.middleware.ts` — 요청 헤더 `X-Internal-Api-Secret`과 timing-safe 비교 후 내부 전용 윈도우·한도 적용 |
| 패턴 | 비우면 내부 버킷을 비활성화하고 공개 IP 레이트 리밋만 적용 |

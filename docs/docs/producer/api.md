...
Producer를 기동한 뒤 Swagger 엔드포인트에 접근합니다(환경에 따라 `/api/docs` 등 — `swagger.config.ts` 참고).

Apidog 등 외부 도구로 `server/producer` Swagger 스펙을 export한 뒤 import할 수 있습니다.
...
- 미들웨어는 `rate-limit.middleware.ts`입니다.
